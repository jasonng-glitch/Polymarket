...
def get_client(settings: Settings) -> ClobClient:
    ...
    print(f"Create client sucess: {_cached_client}")
    return _cached_client
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(description="jlb.py 功能测试")
    parser.add_argument("--action", choices=["balance", "positions", "order"], default="balance")
    ...
    parser.add_argument("--filter-token-id", action="append", default=[])
...
def main():
    ...
    if args.action == "balance":
        ...
        print(f"USDC 余额: {balance:.6f}")
        return

    if args.action == "positions":
        token_ids = args.filter_token_id or None
        positions = get_positions(settings, token_ids=token_ids)
        print(positions)
        return

    if args.action == "order":
        if not args.token_id or args.price <= 0 or args.size <= 0:
            raise ValueError("下单需要 --token-id, --price, --size")
        result = place_order(
            settings,
            side=args.side,
